...
class WindowsExePackagingTests(unittest.TestCase):
    def test_build_script_declares_safe_pyinstaller_shape(self) -> None:
        ...
        self.assertIn("workspace-flow --workspace $SmokeWorkspace", text)
        ...
        self.assertIn("action-request exe_cli_agent --workspace $SmokeWorkspace", text)
        self.assertIn("request-status --workspace $SmokeWorkspace", text)
        self.assertIn("request-run --workspace $SmokeWorkspace", text)
